<template>
    <section class="container" style="overflow:hidden;margin-top:3em;position:relative;min-height: 100vh;">
        <v-row v-if="user.userPhoto">
        <h3 style="width:100%;text-align:center;margin-bottom:1em;" :class="`${user.emotions}`">Fotogalerij</h3>
          <v-col
            v-for="photo in user.userPhoto"
            :key="photo.id"
            class="d-flex child-flex col-sm-12"
          ...
          >
            <v-img
              ...
              :lazy-src="photo.url"
              aspect-ratio="1"
              class="grey lighten-2 images1-image"
              :class="[borderBox ? 'boxShadow0' : '']"
            >
